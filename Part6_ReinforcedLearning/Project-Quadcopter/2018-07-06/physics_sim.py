...
class PhysicsSim():
    def __init__(self, init_pose=None, init_velocities=None, init_angle_velocities=None, runtime=5.):
        ...
        self.upper_bounds = np.array([env_bounds / 2, env_bounds / 2, 200])

        self.reset()

    def reset(self):
        self.time = 0.0
        self.pose = np.array([0.0, 0.0, 100.0, 0.0, 0.0, 0.0]) if self.init_pose is None else np.copy(self.init_pose)
    ...
    def next_timestep(self, rotor_speeds):
        ...
        if self.time > self.runtime:
            print(', Reached {:d} sec'.format(int(self.runtime)))
            self.done = True
        return self.done
