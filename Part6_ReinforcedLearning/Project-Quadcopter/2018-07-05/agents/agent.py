...
class DDPG():
    ...
    def act(self, state):
        ...
        action_noisy = np.array(action + self.noise.sample()*0)
        for k in range(self.action_size):
            action_noisy[k] = np.max([action_noisy[k], self.action_low+1]) # adding 1 to avoid division by 0
            action_noisy[k] = np.min([action_noisy[k], self.action_high])

        return action_noisy  # add some noise for exploration
