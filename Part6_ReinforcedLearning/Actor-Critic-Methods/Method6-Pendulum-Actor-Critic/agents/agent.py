...
class Critic:
    ...
    def build_model(self):
        ...
        if False:  # Udacity Project Quadrotor
            states = layers.Input(shape=(self.state_size,), name='states')
...
class DDPG():
    ...
    def __init__(self, env, state_size, action_size, action_low, action_high):
...
class OUNoise:
    ...
    def sample(self):
        ...
        dt = 1e-2
        dx = self.theta * (self.mu - x) * dt + \
                self.sigma * np.sqrt(dt) * np.random.normal(size=self.mu.shape)
